...
class ReportTestResult(unittest.TestResult, RunTest):
    """ override"""
    ...
    def get_all_result_info_tuple(self, test) -> tuple:
        ...
        """
        # 修改后
        t = self.get_testcase_property(test)
        return tuple([t[0], t[1], t[2], self.end_time, self.status, self.case_log])
    ...
    def addSuccess(self, test) -> None:
        ...
        # 修改
        logs.append('Test Msg:'.center(100, '-'))
        logs.extend(str(RunTest.getCaseMsg(test)).split('\\n'))
        self.add_test_type('成功', logs)
        # print(RunTest.getCaseMsg(test))  #  自定义方法，返回测试的请求及响应基本信息

        self._mirrorOutput = True  # print(class_name, method_name, method_doc)  # 默认
    ...
    def addError(self, test, err):
        ...
        output = self.complete_output()
        logs.append(output)
        logs.append('Test Msg:'.center(100, '-'))  # 修改
        logs.extend(str(RunTest.getCaseMsg(test)).split('\\n'))  # 修改
        logs.append('Error Msg:'.center(100, '-'))  # 修改
    ...
    def addFailure(self, test, err):
        ...
        logs.append('Test Msg:'.center(100, '-'))  # 修改
        logs.extend(str(RunTest.getCaseMsg(test)).split('\\n'))  # 修改
        logs.append('Failure Msg:'.center(100, '-'))  # 修改
        logs.extend(self.error_or_failure_text(err))
        self.failure_count += 1
        self.add_test_type('失败', logs)
        if self.verbosity > 1:
